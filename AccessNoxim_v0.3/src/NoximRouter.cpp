...
int X_diff, Y_diff, Z_diff = 0;
int upport_wt, downport_wt, northport_wt, southport_wt, eastport_wt, westport_wt = INITIAL_WEIGHT;
...
vector<int> NoximRouter::routingAdaptive3D(const NoximCoord & current, const NoximCoord & source, const NoximCoord & destination) {
	...
	if (-1 <= X_diff <= 1 && -1 <= Y_diff <= 1 && -1 <= Z_diff <= 1) {
		if (X_diff > 0) { eastport_wt = HORIZONTAL_CLOSE; }
		else if (X_diff < 0) { westport_wt = HORIZONTAL_CLOSE; }

		if (Y_diff > 0) { southport_wt = HORIZONTAL_CLOSE; }
		else if (Y_diff < 0) { northport_wt = HORIZONTAL_CLOSE; }

		if (Z_diff > 0) { downport_wt = VERTICAL_CLOSE; }
		else if (Z_diff < 0) { upport_wt = VERTICAL_CLOSE; }

	}
	else {
		if (X_diff > 0) { eastport_wt = HORIZONTAL_FAR_MIN; westport_wt = HORIZONTAL_FAR_DETOUR; }
		else if (X_diff < 0) { eastport_wt = HORIZONTAL_FAR_DETOUR; westport_wt = HORIZONTAL_FAR_DETOUR; }

		if (Y_diff > 0) { southport_wt = HORIZONTAL_FAR_MIN; northport_wt = HORIZONTAL_FAR_DETOUR; }
		else if (Y_diff < 0) { southport_wt = HORIZONTAL_FAR_DETOUR; northport_wt = HORIZONTAL_FAR_DETOUR; }

		if (Z_diff > 0) { downport_wt = VERTICAL_FAR; }
	...
	if (X_diff == Y_diff == Z_diff == 0) {
		...
		for (int i = 0;i < 6;i++) {
			...
			Traffic_condition[i] = free_slots[i] * wts[i];
		}
		int swap_holder;
		int swap_direction;
		int temp_direction[] = { 0,1,2,3,4,5 };
		int swapwt;
		for (int i = 0; i < 6;i++) {
			for (int j = i + 1;j < 6;j++)
				if (Traffic_condition[i] < Traffic_condition[j]) {
					swap_holder = Traffic_condition[i];
					Traffic_condition[i] = Traffic_condition[j];
					Traffic_condition[j] = swap_holder;
					swapwt = wts[i];
					wts[i] = wts[i];
					wts[j] = swapwt;
					swap_direction = temp_direction[i];
					temp_direction[i] = temp_direction[j];
					temp_direction[j] = swap_direction;
				}
		}

		if (Traffic_condition[0] > Traffic_condition[1]) {
			directions.push_back(temp_direction[0]);
			return directions;
		}

		else if (Traffic_condition[0] == Traffic_condition[1]) {
			int a[6];
			a[0] = wts[0];
			a[1] = wts[1];
			for (int i = 1;i < 6;i++) {
				if (Traffic_condition[i] == Traffic_condition[i + 1])
					a[i + 1] = wts[i + 1];
				else break;
			}
			for (int i = 0; i < 6;i++) {
